...
def _blazed_script(project_dir, python_path):
    ...
    tpl.append("    # Preserve user data (config + storage)")
    tpl.append("    cp -r \"$PROJECT_DIR/config\" \"$tmp/bak_cfg\" 2>/dev/null")
    tpl.append("    cp -r \"$PROJECT_DIR/storage\" \"$tmp/bak_sto\" 2>/dev/null")
    tpl.append("    # Also preserve any custom soul.md or skills.yaml the user may have edited")
    tpl.append("    [ -f \"$PROJECT_DIR/config/soul.md\" ] && cp \"$PROJECT_DIR/config/soul.md\" \"$tmp/bak_soul.md\" 2>/dev/null")
    tpl.append("    [ -f \"$PROJECT_DIR/config/skills.yaml\" ] && cp \"$PROJECT_DIR/config/skills.yaml\" \"$tmp/bak_skills.yaml\" 2>/dev/null")
    tpl.append("    # Verify zip integrity")
    tpl.append("    python3 -c \"import zipfile; zipfile.ZipFile('$tmp/blaze-agent.zip').testzip()\" 2>/dev/null || {")
    tpl.append("      echo \"  Error: Download is corrupt.\"; rm -rf \"$tmp\"; exit 1; }")
    tpl.append("    unzip -qo \"$tmp/blaze-agent.zip\" -d \"$tmp/ext\"")
    tpl.append("    # Detect whether zip has a wrapper folder or files at root")
    tpl.append("    top_count=$(ls -1 \"$tmp/ext\" | wc -l)")
    tpl.append("    top_item=$(ls -1 \"$tmp/ext\" | head -1)")
    tpl.append("    if [ \"$top_count\" -eq 1 ] && [ -d \"$tmp/ext/$top_item\" ]; then")
    tpl.append("      # Single wrapper folder: merge its contents")
    tpl.append("      src=\"$tmp/ext/$top_item\"")
    tpl.append("    else")
    tpl.append("      # Multiple root files: merge from ext dir itself")
    tpl.append("      src=\"$tmp/ext\"")
    tpl.append("    fi")
    tpl.append("    # Merge: copy new files, skip preserved dirs/files")
    tpl.append("    cd \"$src\"")
    tpl.append("    for item in *; do")
    tpl.append("      case \"$item\" in")
    tpl.append("        config|storage) ;;  # skip, will restore from backup")
    tpl.append("        *) cp -r \"$item\" \"$PROJECT_DIR/\" ;;")
    tpl.append("      esac")
    tpl.append("    done")
    tpl.append("    cd \"$PROJECT_DIR\"")
    tpl.append("    # Restore preserved user data")
    tpl.append("    [ -d \"$tmp/bak_cfg\" ] && cp -r \"$tmp/bak_cfg/.\" \"$PROJECT_DIR/config/\"")
    tpl.append("    [ -d \"$tmp/bak_sto\" ] && cp -r \"$tmp/bak_sto/.\" \"$PROJECT_DIR/storage/\"")
    tpl.append("    [ -f \"$tmp/bak_soul.md\" ] && cp \"$tmp/bak_soul.md\" \"$PROJECT_DIR/config/soul.md\"")
    tpl.append("    [ -f \"$tmp/bak_skills.yaml\" ] ] && cp \"$tmp/bak_skills.yaml\" \"$PROJECT_DIR/config/skills.yaml\"")
    tpl.append("    # Merge new requirements.txt (keep old if no new one)")
    tpl.append("    echo \"  Checking dependencies...\"")
    ...
    tpl.append("    echo \"  Config and storage preserved.\"")
    ...
    tpl.append("    echo \"    uninstall Remove Blaze-Agent\"")
    tpl.append("    echo \"    version   Show version\"")
    tpl.append("    echo \"    help      This help\"; echo \"\"")
    tpl.append("    ;;")
    tpl.append("  uninstall)")
    tpl.append("    force=\"\"")
    tpl.append("    [ \"$2\" = \"--yes\" ] && force=\"y\"")
    tpl.append("    echo \"\"")
    tpl.append("    echo \"  This will remove Blaze-Agent from this machine.\"")
    tpl.append("    echo \"  Project dir: $PROJECT_DIR\"")
    tpl.append("    echo \"  The blzed command will also be removed.\"")
    tpl.append("    echo \"\"")
    tpl.append("    printf \"  Are you sure? [y/N]: \"")
    tpl.append("    if [ -n \"$force\" ]; then")
    tpl.append("      confirm=\"$force\"")
    tpl.append("      echo \"$confirm\"")
    tpl.append("    else")
    tpl.append("      read confirm")
    tpl.append("    fi")
    tpl.append("    case \"$confirm\" in")
    tpl.append("      y|Y)")
    tpl.append("        # Remove blzed command")
    tpl.append("        rm -f \"$HOME/.local/bin/blzed\" 2>/dev/null")
    tpl.append("        rm -f \"$HOME/.local/bin/blzed.cmd\" 2>/dev/null")
    tpl.append("        # Remove PATH entry from shell rc")
    tpl.append("        for rc in \"$HOME/.bashrc\" \"$HOME/.zshrc\"; do")
    tpl.append("          [ -f \"$rc\" ] && sed -i '/# Blaze-Agent blzed CLI/d' \"$rc\" 2>/dev/null")
    tpl.append("          [ -f \"$rc\" ] && sed -i '/\\.local\\/bin/d' \"$rc\" 2>/dev/null")
    tpl.append("        done")
    tpl.append("        # Remove project directory")
    tpl.append("        rm -rf \"$PROJECT_DIR\"")
    tpl.append("        echo \"\"")
    tpl.append("        echo \"  Blaze-Agent removed.\"")
    tpl.append("        echo \"  Open a new terminal for PATH changes to take effect.\"")
    tpl.append("        echo \"\"")
    tpl.append("        ;;")
    tpl.append("      *)")
    tpl.append("        echo \"  Cancelled.\"")
    tpl.append("        ;;")
    tpl.append("    esac")
    tpl.append("    ;;")
